	...
	export default {
		...
		mounted () {
			...
			for ( let i = 0; i < scripts.length; i++ ) {
				if ( scripts[i].getAttribute('data-id') ) arr.push(scripts[i].getAttribute('data-id'))
			}
			if ( arr.indexOf('ybbarrage') == -1 ) {
				const script2 = document.createElement('SCRIPT')
				script2.setAttribute('data-id', 'ybbarrage')
				script2.src = './uni_modules/yingbing-video/static/yingbing-video/ybbarrage.js'
				script2.onload = () => {
					this.switchVideo()
				}
				document.body.appendChild(script2)
			}
			if ( arr.indexOf('hls') == -1 ) {
				const script3 = document.createElement('SCRIPT')
				script3.setAttribute('data-id', 'hls')
				script3.src = './uni_modules/yingbing-video/static/yingbing-video/hls.min.js'
				script3.onload = () => {
					this.switchVideo()
				}
				document.body.appendChild(script3)
			}
			if ( arr.indexOf('flv') == -1 ) {
				const script4 = document.createElement('SCRIPT')
				script4.setAttribute('data-id', 'flv')
				script4.src = './uni_modules/yingbing-video/static/yingbing-video/flv.min.js'
				script4.onload = () => {
					this.switchVideo()
				}
				document.body.appendChild(script4)
			}
			if ( arr.indexOf('ybplayer') == -1 ) {
				const script1 = document.createElement('SCRIPT')
				script1.setAttribute('data-id', 'ybplayer')
				script1.src = './uni_modules/yingbing-video/static/yingbing-video/ybplayer.js'
				script1.onload = () => {
					this.switchVideo()
				}
