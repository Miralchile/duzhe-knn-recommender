		...
		<view class="hb-comment">
			...
			<view class="comment-list" v-if="items.length != 0">
				...
				<view class="comment-num">
					<view>共 {{items.length}} 条回答</view>
					<view class="add-btn">
						<button type="primary" size="mini" @click="commentInput">回答提问</button>
					</view>
				</view>
						...
						<view class="comment-main">
							...
							<view class="comment-main-top">
								<view class="nick-name-box">
									<view class="comLogo com3" v-if="isQuestioner"
									 @click="clickAdd(item.content,item.answerer,item.createdBy)">
									 设为最佳</view>
			...
			<view class="comment-none" v-else>
				暂无答案，<span @click="commentInput" style="color: #007AFF;">去回答</span>
			</view>
			<!-- 无评论-end -->
			<!-- 新增评论-start -->
...
<script>
	import {
		getAnswerById, addAnswerById, addDiscussionBestAnswer, checkIsQuestioner
	...
	export default {
		...
		mounted(){
			checkIsQuestioner(this.message).then((res) => {
				this.isQuestioner = res 
				getAnswerById(this.message).then((res) => {
		...
		methods:{
			...
			clickAdd(content,answerer,createdBy) {
				this.addBestAnswerForm.id = this.message
				this.addBestAnswerForm.content = content
				this.addBestAnswerForm.answerer = answerer
				this.addBestAnswerForm.createdBy = createdBy
				addDiscussionBestAnswer(this.addBestAnswerForm).then(()=>{
					uni.showToast({
						title: '设置成功',
						icon: 'success'
					})
