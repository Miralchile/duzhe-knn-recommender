						...
						<view class="card2" v-for="(item,index) in items" :key="index">
							...
							z-index: 5;height: 15%;width: 25%;background-color: black;opacity: 0.5;" align="center"></u--text>
								...
								<view>
									<u--text :text="item.title" style="position: relative; top:13rpx;left: 15rpx;"
										size="13"></u--text>
									<view style="margin-top:17rpx; z-index: 5;">
							...
							<view v-if="item.type == 2" style="height: 100%;" @click="gotoImage(item._id)">
								<view style="width: 100%;height: 65%;">
									...
									<uni-tag v-show="loadStatus[index]" :circle="true" text="图文" type="primary" style="position: relative;left:255rpx;bottom: 98rpx;
...
<script>
	...
	import hotIndex from '../hot/index.vue';
	import youScroll from '@/components/you-scroll.vue'
	import discussIndex from '../discussion/index.vue'
	import {
		getRecommendList, getRecommendListByCategory
	} from '@/api/homePage'
	...
	export default {
		...
		onLoad() {
			getRecommendList(this.pageNo).then((res) => {
				this.items = res;
				console.log(res)
		...
		methods: {
			...
			loadDate(){
				getRecommendListByCategory(this.pageNo,this.tabs[this.current]).then((res) => {
					for (var i = 0; i < 7; i++) {
						this.$set(this.loadStatus, i, false);
					}
					this.other = res;
