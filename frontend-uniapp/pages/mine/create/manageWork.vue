	...
	<view>
		<view class="top2" >
			<view class="card2" v-for="(item,index) in items" :key="index">
				<view v-if="item.type == 1" style="height: 100%;" @click="manageVideoWork(item._id)">
				...
				<view v-if="item.type == 2" style="height: 100%;" @click="manageImageWork(item._id)">
					<view style="width: 100%;height: 65%;">
						<view v-if="!loadStatus[index]" class="loading-container">
							<i class="loading-icon"></i>
						</view>
		...
		<view>
			<u-modal :show="showConfirmDelete" content='是否删除此作品' @cancel="showConfirmDelete=false"
			showCancelButton confirmText="确认删除" confirmColor="#ff0000" @confirm="confirmDeleteWorkById(currentId)"></u-modal>
		</view>
		<view>
			<u-popup :show="show" @close="close">
			    <view>
					<u-button v-if="currentType == 1" type="success" :plain="true" :hairline="true"
					 text="浏览" @click="gotoVideo(currentId)"></u-button>
					<u-button v-if="currentType == 2" type="success" :plain="true" :hairline="true"
					 text="浏览" @click="gotoImage(currentId)"></u-button>
			        <u-button v-if="currentType == 1" type="primary" :plain="true" :hairline="true"
					 text="编辑" @click="gotoEditVideo(currentId)"></u-button>
					<u-button v-if="currentType == 2" type="primary" :plain="true" :hairline="true"
					 text="编辑" @click="gotoEditImage(currentId)"></u-button>
					<u-button type="warning" :plain="true" :hairline="true" text="删除" @click="clickDeleteWorkById(currentId)"></u-button>
					<u-button type="info" :plain="true" :hairline="true" text="取消" @click="close()"></u-button>
			    </view>
			</u-popup>
		</view>
	</view>
...
<script>
	import { getCurrentUser, getMyWorkList, getMyWorkListTotal, deleteWorkById } from '@/api/mine.js'
	...
	export default {
		...
		onLoad(option) {
			getMyWorkList(this.page.currentPage,this.page.pageSize).then((res)=>{
				this.items = res
			...
			getMyWorkListTotal().then((res)=>{
				this.page.total = res
			});
		},
		methods: {
			clickDeleteWorkById(id){
				this.show = false
				this.showConfirmDelete = true
			},
			confirmDeleteWorkById(id){
				this.show = false
				this.showConfirmDelete = false
				deleteWorkById(id).then(()=>{
					uni.showToast({
						title: "删除成功",
					})
					getMyWorkList(this.page.currentPage,this.page.pageSize).then((res)=>{
			...
			gotoEditVideo(id){
				this.show = false
				uni.navigateTo({
					url: '/pages/mine/create/videoCreate?id=' + id
				});
			},
			gotoEditImage(id){
				this.show = false
				uni.navigateTo({
					url: '/pages/mine/create/imageCreate?id=' + id
				});
			},
			gotoVideo(id) {
				this.show = false
			...
			change(currentPage, type){
				getMyWorkList(currentPage,this.page.pageSize).then((res) => {
					this.items = res
					for (var i = 0; i < this.items.length; i++) {
						var duration = this.items[i].duration
						var seconds = Math.floor(duration / 1000);
