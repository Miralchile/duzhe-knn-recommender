			...
			<view class="talk-list" :style="{'padding-bottom':keyboardHeight+'rpx'}">
				<view class="message-list" v-for="(item,index) in groupMessage[s_index].list" :key="index" :id="`msg-${item.id}`">
					...
					<view v-if="item.isWithdrawn===0 && item.isShowGroupMessage == true " class="item flex_col" :class=" item.senderId == loginUserInfo.userId ? 'push':'pull' ">	
						<image @tap="gotoPersonInfo(item.senderId)" :src="item.senderId == loginUserInfo.userId ? 'http://localhost:8080'+loginUserInfo.avatar:'http://localhost:8080'+item.avatar" mode="aspectFill" class="pic"></image>
						<view style="position: relative;">
							<text :class="item.senderId == loginUserInfo.userId ? 'message-name-right':'message-name-left'" style="color: #BEBEBE;white-space: nowrap;">{{item.senderName}}</text>
