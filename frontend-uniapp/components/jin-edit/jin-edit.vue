	...
	<view class="container" :style="{height: height}"> 
		...
		<view class="tool-view" > 
			...
			<view class="font-more" :style="{ height: showMoreTool ? '100rpx' : 0 }">
				...
				<jinIcon class="single" type="&#xe6ee;" font-size="44rpx" title="居左" @click="setLeft" :color="showLeft ? activeColor : '#666666'"></jinIcon>
			...
			<view class="tool">
				...
				<addIcon class="single" type="&#xe601;" font-size="44rpx" title="代码" @click="setCode"></addIcon>
...
import addIcon from './add-icons.vue';
...
export default {
	...
	methods: {
			...
			uni.chooseImage({
				...
				success: async(res) => {
					...
					for (let temp of tempFilePaths) {
						...
						// 图片上传服务器
						await uploadImage(temp).then(res=>{
								// 上传完成后处理
								this.editorCtx.insertImage({
									src: JSON.parse(res[1].data).imagePath,  // 此处需要将图片地址切换成服务器返回的真实图片地址
									alt: '图片',
									success: function(e) {}
								});
								uni.hideLoading()
		...
		setCode(){
			const codeContent = '<pre><code>' + 'console.log("Hello, world!")' + '</code></pre>';
			this.editorCtx.insertText(codeContent);
		},
		setItalic() {
		...
		setLeft() {
			...
			this.editorCtx.format('align', this.showLeft ? 'left' : false);
		},
		setCenter() {
